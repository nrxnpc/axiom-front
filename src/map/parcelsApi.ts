...
export async function fetchParcelsByBounds(bbox: [number, number, number, number]): Promise<HeatPoint[]> {
  await new Promise((r) => setTimeout(r, MOCK_DELAY_MS));
  const padded = addPadding(bbox);
  const all = getMockParcelsAll();
  return filterPointsByBounds(all, padded);
}

const MOCK_SCAN_DELAY_MS = 150 + Math.floor(Math.random() * 100);

export async function fetchScansByBounds(bbox: [number, number, number, number]): Promise<HeatPoint[]> {
